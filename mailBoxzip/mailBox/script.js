...
filterMenu.addEventListener('click', (e) => {
    ...
    if (activeFilter === 'unread') {
        ...
        visibleEmails.sort((a, b) => new Date(b.date) - new Date(a.date));
    } else if (activeFilter === 'oldest') {
        visibleEmails.sort((a, b) => new Date(a.date) - new Date(b.date));
    }
...
function applyFilter(filter) {
    if (filter === "unread") {
        visibleEmails = allEmails.filter(e => !e.isRead);
